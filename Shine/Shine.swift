...
//  Modified by Le Hai Trieu on 23/09/20
...
fileprivate extension Shine {
    private struct UserDefaultKeys {
        ...
        static let SkipToNextVersion = "Shine:SkipToNextVersion"
    }
    ...
    var skipToNextVersion: Bool {
        get {
            return UserDefaults.standard.bool(forKey: UserDefaultKeys.SkipToNextVersion)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: UserDefaultKeys.SkipToNextVersion)
            UserDefaults.standard.synchronize()
        }
    }
}
...
@objcMembers public class Shine: NSObject {
        ...
        self.backgroundQueue.async {
            do {
                ...
                guard let currentVersionCode = Bundle.main.infoDictionary?["CFBundleVersion"] as? String else {
                    assertionFailure("Shine: CFBundleVersionが必要です。")
                    return
                }
                ...
                if newItem != nil {
                    ...
                    if self.skipToNextVersion == false || newItem!.versionCode.isNewerThanVersion(self.lastCheckLatestVersion ?? currentVersionCode) {
                        self.skipToNextVersion = false
                    ...
                    if forceNotify {
                        DispatchQueue.main.sync {
                            ...
                            let noUpdateAlert = UIAlertController(title: "お使いのアプリは最新です。", message: "\(bundleDisplayname ?? "バージョン") \(displayVersion ?? currentVersionCode)は最新のバージョンです。", preferredStyle: .alert)
                            noUpdateAlert.addAction(UIAlertAction(title: "はい", style: .cancel, handler: nil))
    ...
    private func notifyUserOfUpdate(toVersion: AppCastItem, force: Bool) {
        ...
        let updateStatement = (toVersion.forcedUpdate) ? "更新しないといけないバージョンです!" : "すぐに更新しましょうか？"
        
        let updateAlert = UIAlertController(title: "\(bundleDisplayname ?? "このアプリ")の更新があります!", message: "最新：\(bundleDisplayname ?? "") \(toVersion.displayVersion ?? toVersion.versionCode) ー 現在：\(displayVersion ?? currentVersionCode)\n\(updateStatement)\(releaseNotes)", preferredStyle: .alert)
        updateAlert.messageLabel?.textAlignment = .center
        
        updateAlert.addAction(UIAlertAction(title: "更新", style: .default) { _ in
            self.beginUpdateProcess(toVersion: toVersion)
        })
        
        if !toVersion.forcedUpdate {
            updateAlert.addAction(UIAlertAction(title: "後で", style: .cancel) { _ in
                self.selectedRemindLaterDate = Date()
            })
            updateAlert.addAction(UIAlertAction(title: "スキップ", style: .default) { _ in
                self.skipToNextVersion = true
            })
        }
    ...
    private func beginUpdateProcess(toVersion: AppCastItem) {
        ...
        #else
        let moveToWeb = toVersion.moveToWeb
        let downloadTriggerURL: URL
        let givenURL = toVersion.appURL
        if givenURL.scheme == "itms-services" || moveToWeb {
...
fileprivate struct AppCastItem: XMLIndexerDeserializable {
    ...
    static func deserialize(_ element: XMLIndexer) throws -> AppCastItem {
        ...
        return try AppCastItem(title: element["title"].value(),
                           ...
                           moveToWeb: element["moveToWeb"].element != nil)
    }
}
